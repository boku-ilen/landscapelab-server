...
def land_use_to_splatmap(path_to_land_use):
    ...
    as well as a set with all IDs in this splatmap.
    """

    # Load land use map
    land_use_image = Image.open(path_to_land_use)
    land_use_width_pixels, land_use_height_pixels = land_use_image.size
    land_use_width_meters = land_use_width_pixels / LAND_USE_PIXELS_PER_METER
    land_use_height_meters = land_use_height_pixels / LAND_USE_PIXELS_PER_METER

    land_use_pixels = land_use_image.load()

    splatmap_width_pixels = int(land_use_width_meters * SPLATMAP_PIXELS_PER_METER)
    splatmap_height_pixels = int(land_use_height_meters * SPLATMAP_PIXELS_PER_METER)
    ...
    for x in range(splatmap_width_pixels):
        for y in range(splatmap_height_pixels):
            ...
            # Map the pixel to the corresponding phytocoenosis ID
            # TODO: Replace with more sophisticated method
...
def get_splatmap_pixel_for_land_use_pixel(land_use_pixel):
    ...
    If the value is present in the land use color to phytocoenosis ID map, the correct
    phytocoenosis ID is entered. Otherwise, the returned splatmap pixel is 0, meaning no
    phytocoenosis is present there.

    Currently a 1:1-mapping without using additional parameters - this function will probably
    be replaced once the mapping becomes more sophisticated.
    """
...
def splatmap_to_land_use_coords(x, y):
    """Translates splatmap x and y pixel coordinates to the corresponding land use
